...
class EventPublisher:
    ...
    def get_events_for_aggregate(self, aggregate_id: str) -> list:
        """Récupère tous les événements pour un agrégat donné"""
        if not self.mongo_available or self.event_store is None:
            logger.warning("MongoDB not available, cannot retrieve events")
            return []
            
        events = list(self.event_store.find(
            {"aggregate_id": aggregate_id}
        ).sort("timestamp", 1))
    ...
    def replay_events(self, 
                     aggregate_id: str, 
                     up_to_timestamp: Optional[str] = None) -> list:
        """Rejoue les événements pour reconstituer un état"""
        if not self.mongo_available or self.event_store is None:
            logger.warning("MongoDB not available, cannot replay events")
            return []
            
        query = {"aggregate_id": aggregate_id}
        
        if up_to_timestamp:
            query["timestamp"] = {"$lte": up_to_timestamp}
            
        events = list(self.event_store.find(query).sort("timestamp", 1))
